...
def matrix_eigen_window(a11, tau1, a12, a21, a22, tau2, a13, a23, a31, a32, a33, tau3, a14, a24, a34, a41, a42, a43, a44, tau4):
    global num_compartments
    window2 = tk.Toplevel()
    window2.title('Data')
    # mass transfer matrix 
    m_matrix = np.array([[a11, a12, a13, a14],
                        [a21, a22, a23, a24],
                        [a31, a32, a33, a34],
                        [a41, a42, a43, a44]])
    m_adj = np.zeros((num_compartments, num_compartments))
    for i in range(num_compartments):
        for j in range(num_compartments):
            m_adj[i, j] = m_matrix[i,j]
    time_const = np.array([tau1, tau2, tau3, tau4])
    time_adj = np.zeros(num_compartments)
    for i in range(num_compartments):
        time_adj[i] = time_const[i]
    # calculate eigenvalues and eigenvectors
    eigenvalues, eigenvectors = np.linalg.eig(m_adj)
    ...
    tk.Label(window2, text="Mass Transfer Matrix", font=("Arial", 16)).grid(row=0, column=0, padx=10, pady=10)
    tk.Label(window2, text=m_matrix[:num_compartments, :num_compartments]).grid(row=1, column=0, padx=10, pady=10)
    tk.Label(window2, text='Eigenvalues', font=("Arial", 16)).grid(row=2, column=0, padx=10, pady=10)
    tk.Label(window2, text=eigenvalues).grid(row=3, column=0, padx=10, pady=10)
    tk.Label(window2, text='Eigenvectors', font=("Arial", 16)).grid(row=4, column=0, padx=10, pady=10)
    tk.Label(window2, text=eigenvectors).grid(row=5, column=0, padx=10, pady=10)
    tk.Label(window2, text='Time Constants', font=("Arial", 16)).grid(row=6, column=0, padx=10, pady=10)
...
def func1(t, a11=0, a12=0, a13=0, a14=0, tau1=0, tau2=0, tau3=0, tau4=0):
    ...
    result = a11 * (np.exp(-t/tau1) if tau1 != 0 else 0) + \
...
def func2(t, a21=0, a22=0, a23=0, a24=0, tau1=0, tau2=0, tau3=0, tau4=0):
    # Check if any tau values are zero
    result = a21 * (np.exp(-t/tau1) if tau1 != 0 else 0) + \
...
def func3(t, a31=0, a32=0, a33=0, a34=0, tau1=0, tau2=0, tau3=0, tau4=0):
    # Check if any tau values are zero
    result = a31 * (np.exp(-t/tau1) if tau1 != 0 else 0) + \
...
def func4(t, a41=0, a42=0, a43=0, a44=0, tau1=0, tau2=0, tau3=0, tau4=0):
    # Check if any tau values are zero
    result = a41 * (np.exp(-t/tau1) if tau1 != 0 else 0) + \
...
def comboFunc(t, a11=0, tau1=0, a12=0, a21=0, a22=0, tau2=0, a13=0, a23=0, a31=0, a32=0, a33=0, tau3=0, a14=0, a24=0, a34=0, a41=0, a42=0, a43=0, a44=0, tau4=0):
    r1 = func1(t=t, a11=a11, a12=a12, a13=a13, a14=a14,
               tau1=tau1, tau2=tau2, tau3=tau3, tau4=tau4)
    r2 = func2(t=t, a21=a21, a22=a22, a23=a23, a24=a24,
               tau1=tau1, tau2=tau2, tau3=tau3, tau4=tau4)
    r3 = func3(t=t, a31=a31, a32=a32, a33=a33, a34=a34,
               tau1=tau1, tau2=tau2, tau3=tau3, tau4=tau4)
    r4 = func4(t=t, a41=a41, a42=a42, a43=a43, a44=a44,
               tau1=tau1, tau2=tau2, tau3=tau3, tau4=tau4)
...
def run_simulation():
    ...
    a11, tau1, a12, a21, a22, tau2, a13, a23, a31, a32, a33, tau3, a14, a24, a34, a41, a42, a43, a44, tau4 = fittedparam_extended
    ...
    matrix_eigen_window(a11, tau1, a12, a21, a22, tau2, a13, a23, a31, a32, a33, tau3, a14, a24, a34, a41, a42, a43, a44, tau4)
    ...
    for i in range(num_compartments):
        ...
        plt.scatter(df.iloc[:, 0], df.iloc[:, i+1], s=6, marker='o',
                    label=f'Compartment {i+1}', color=colors[i % len(colors)])
...
settings_compartments = ttk.LabelFrame(
    frame_main, padding="10", text="Settings")
...
comp_number = OptionMenu(settings_compartments, nCompartments,
